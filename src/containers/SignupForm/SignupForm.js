...
practitionerDetailCheck = () => {
  ...
  render(){
    ...
    let pagination = null;
    const {username, newPassword, confirmPassword, firstName, lastName, email, dob, gender, title, serviceProvided, abn,
      medicareProNum, accreditedBodies, businessName, businessAddress,selectedImg, stripeToken, bundle, verificationErr, agreement, errors} = this.state;
    switch(this.state.pageNo){
      case(0):
          currentPage = <div className={styles.Border}>
                                <Row><Col xs={1} md={2} lg={3}></Col><Col xs={10} md={8} lg={6}>
                                          <AccountPage
                                          email={email}
                                          username={username}
                                          ...
                                          onBlur={this.onBlur}
                                          next={this.nextPageHandler} /></Col></Row></div>;
          pagination = <Row><Col sm={6}></Col><Col sm={6}  ><a className={[buttons.btn, buttons.btngreen].join(' ')}
              onClick={this.nextPageHandler}>next</a></Col></Row>;
        break;
      case(1):
          currentPage = <div className={styles.Border}><PractitionerPage
                                          ...
                                          onBlur={this.onBlur}
                                          /></div>;
          pagination = <Row><Col sm={6}><a className={[buttons.btn, buttons.btngreen].join(' ')}
                onClick={this.previousPageHandler}>prev</a></Col><Col sm={6}><a className={[buttons.btn, buttons.btngreen].join(' ')}
                onClick={this.nextPageHandler}>next</a></Col></Row>;
        break;
      case(2):
          currentPage = <div className={styles.Border}>
                              <StripeProvider apiKey="pk_test_OUqtPerqAmIdMxbK8PagM3Ng">
                                <Elements>
      ...
      case(3):
          ...
          pagination = <Row><Col sm={6}><a className={[buttons.btn, buttons.btngreen].join(' ')}
                onClick={this.previousPageHandler}>prev</a></Col><Col sm={6}><button className={[buttons.btn, buttons.btngreen].join(' ')}
                onClick={this.nextPageHandler} disabled={agreement}>confirm</button></Col></Row>;
        break;
      case(4):
          currentPage = <div className={styles.Border}><ConfirmationPage /></div>;
        break;
      default:
          currentPage = <div className={styles.Border}><AccountPage  next={this.nextPageHandler}/></div>;
        }
