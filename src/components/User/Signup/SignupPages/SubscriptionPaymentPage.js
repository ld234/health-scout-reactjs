...
class SubscriptionPaymentPage extends Component{
  ...
  async handleSubmit () {
    if(this.state.selected != 3 && this.state.validCardCvc && this.state.validCardNumber && this.state.validCardExpiry)
    {
  ...
  onSelectHandler = (selected) =>{
    ...
    if(selected < 3){
      ...
      newClassAttr = selectedPackage.map((bundle, i)=>{
        return i==selected ? bundle.classAttr=bundle.default.concat(" SPactive") : bundle.classAttr=bundle.default
      });
    }
    else if(selected == 3){
      this.setState({selected});
      this.props.setBundle(this.state.selectedPackage[selected].name)
      newClassAttr = selectedPackage.map((bundle, i)=>{
        return i==3 ? bundle.classAttr=bundle.default.concat(" SPactiveSub") : bundle.classAttr=bundle.default
      });
    }
    else{
  ...
  render(){
    ...
    creditCard = <div className="SubOuterCreditCard">
    ...
    return(
      <div className="SPContainer">
        <div className="row">
          <div className={this.state.selectedPackage[3].classAttr} onClick={()=>this.onSelectHandler(3)}>Only Subscription</div>
        </div>
        ...
        <div className="row">
           ...
           <div className={this.state.selectedPackage[0].classAttr} onClick={()=>this.onSelectHandler(0)} >
             <div className="SPinner_container">
               ...
               <div className="SPcontainer_text">
                 <div className="SPcontainer_text1"></div>
         ...
         <div className="row"><div className="col-sm-6"><Button className="btn btn-lg btn-block" color="blue"
                onClick={this.props.prev}>Prev</Button></div><div className="col-sm-6"><Button className="btn btn-lg btn-block" color="blue"
                onClick={this.handleSubmit}>Next</Button></div></div>;
      </div>
    );
  }
}
